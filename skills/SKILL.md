...
2. **以原文为依据**：每条意见必须能定位到稿件原文。修改意见凡引用稿件中的具体内容，原文有行号时指明行号；原文没有行号时指明所在章节与段落（如 In the second paragraph of 2.1 Participants, ...）。定位信息融入意见句子里自然带出即可，不必单独罗列成「章节 / 图表 / 行号」的整齐标签。区分「事实性/方法学错误」与「建议改进」，前者明确断言，后者用建议语气。
...
2. 随后用数字编号列出修改意见，先 Major 后 Minor。每条意见写成 1–3 句的精炼段落，定位信息融入句子里自然带出（如 In the second paragraph of 2.1 Participants, ...），而非单独罗列「章节 / 图表 / 行号」标签；随后一句点到问题为何重要，再给出修改方向。原因解释点到即止，不必替作者展开论证；如涉及可查证资料，可引导作者自行查阅（如 consult CONSORT 2010 Item 8a），给修改方向留出余地。
...
整体话术应简洁、专业、克制，点到即止、不堆砌背景铺陈；定位信息融入句子自然带出，原因解释凝练，必要时引导作者查阅相关规范或文献，而非详细代劳论证。区分以下三种表述强度：
...
## Major Concerns / 主要问题
1. （一段 1–3 句的精炼文字：以 In the second paragraph of 2.1 Participants, ... 之类的句式自然带出定位与问题 → 一句点到为何重要 → 给出修改方向；原因解释凝练，可引导作者查阅相关规范/文献）
2. ……
...
以下话术是本 skill 的内置模板。只能在稿件确实存在相应问题时使用，并应替换章节、行号、变量和研究情境；不要无差别复制，也不要让示例措辞替代对稿件证据的判断。所有示例统一采用简洁、点到即止的写法：定位信息融入句子自然带出，原因不过度展开，必要时引导作者查阅相关规范或文献。
...
- 总体评价同时包含贡献和主要风险。
- 每条意见均有稿件依据，定位信息已融入句子自然带出，并给出精简的修改方向。
- 意见已按 Major / Minor 分级，每条为 1–3 句精炼段落（定位自然融入、原因点到即止、修改方向明确）；修改意见（Major 与 Minor 合计）未超过 20 条。
