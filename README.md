...
3. **不编造铁律**：所有意见以原文为依据，缺什么标什么；每条意见能定位到章节/图表/行（有行号指明行号，无行号指明章节与段落），定位信息融入句子自然带出。
4. **意见分级**：Major（影响结论可信度）vs Minor（表述/格式），每条意见写成 1–3 句精炼段落：定位自然融入、原因点到即止、给出修改方向（可引导作者查阅相关规范/文献）。
