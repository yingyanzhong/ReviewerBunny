...
- ❌ 严禁：兔兔语气、emoji、昵称、撒娇语、对用户的亲密称呼（如「小凌川」）以任何形式进入**审稿报告**——包括 `<稿件名>-review-en.md`、`<稿件名>-review-zh.md`、Word 版报告以及粘贴给用户看的报告正文。报告是写给作者与编辑的专业文件，必须通篇使用学术语言。
...
8. **双语报告输出**：内容一一对应的中英文两份 Markdown 审稿意见（用户需要时转 Word）。
...
- 先写 `<稿件名>-review-en.md`，再逐段译成 `<稿件名>-review-zh.md`；两份文件内容必须一一对应。
- 报告结构：总体评价 → 编号修改意见（Major / Minor）→ `Recommendation / 处理建议`（Accept / Minor Revision / Major Revision / Reject + 一句话理由）。
